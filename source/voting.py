...
#model1 = ModelTransformerv2(model_name=model_name, n_in=300, nout=300, nhid=100, n_heads=2, dropout=0.75) # nout = bert model hidden dim

graph_encoder1 = GraphTransformer(n_in=300, nout=300, nhid=100, n_heads=2, dropout=0.6)
graph_encoder2 = SuperGAT(n_in=300, nout=300, nhid=768, n_heads=4, dropout=0.6)
graph_encoder3 = GATPerso(n_in=300, nout=300, nhid=1024, n_heads=4, dropout=0.6)
graph_encoder4 = GATwMLP(n_in=300, nout=300, nhid=1024, n_heads=4, dropout=0.6)

Classifier = VotingClassifier(model_name=model_name, graph_encoder1=graph_encoder1, graph_encoder2=graph_encoder2, graph_encoder3=graph_encoder3, graph_encoder4=graph_encoder4)


Classifier.to(device)

MODEL_SURNAME =  'CrossValidation'+Classifier.get_model_surname() + model_name
...
f = open(os.path.join(SAVE_DIR, 'summary_graph_encoder1.txt'), 'w')
f.write(summary_graph_encoder)
f.close()

# Print summary of the graph encoder 2 model
encoder = Classifier.get_graph_encoder2()
x = torch.randn(2000,300).to(device)
edge_index = torch.randint(32, size=(2, 2000)).to(device)
data = Data(x=x, edge_index=edge_index)
summary_graph_encoder = summary(encoder, data)
print(summary_graph_encoder)
f = open(os.path.join(SAVE_DIR, 'summary_graph_encoder2.txt'), 'w')
f.write(summary_graph_encoder)
f.close()

# Print summary of the graph encoder 3 model
encoder = Classifier.get_graph_encoder3()
x = torch.randn(2000,300).to(device)
edge_index = torch.randint(32, size=(2, 2000)).to(device)
data = Data(x=x, edge_index=edge_index)
summary_graph_encoder = summary(encoder, data)
print(summary_graph_encoder)
f = open(os.path.join(SAVE_DIR, 'summary_graph_encoder3.txt'), 'w')
f.write(summary_graph_encoder)
f.close()

# Print summary of the graph encoder 4 model
encoder = Classifier.get_graph_encoder4()
x = torch.randn(2000,300).to(device)
edge_index = torch.randint(32, size=(2, 2000)).to(device)
data = Data(x=x, edge_index=edge_index)
summary_graph_encoder = summary(encoder, data)
print(summary_graph_encoder)
f = open(os.path.join(SAVE_DIR, 'summary_graph_encoder4.txt'), 'w')
f.write(summary_graph_encoder)
f.close()
...
summary_text_encoder = summary(Classifier.get_text_encoder(), input_ids, attention_mask)
...
# Load a checkpoint
#save_path = os.path.join(SAVE_DIR, 'model_4.pt')
...
for fold, (train_index, val_index) in enumerate(kf.split(train_dataset)):
    ...
    for i in tqdm(range(nb_epochs)):
        ...
        for batch in train_fold_loader:
            ...
            x_graph1,x_graph2, x_graph3, x_graph4, x_text = Classifier(graph_batch.to(device), 
                                    input_ids.to(device), 
                                    attention_mask.to(device))
            x_graph = (x_graph1+x_graph2+x_graph3+x_graph4)/4
...
graph_model1 = Classifier.get_graph_encoder1()
graph_model2 = Classifier.get_graph_encoder2()
graph_model3 = Classifier.get_graph_encoder3()
graph_model4 = Classifier.get_graph_encoder4()
text_model = Classifier.get_text_encoder()
...
for batch in test_loader:
    for output in graph_model1(batch.to(device)):
        graph_embeddings1.append(output.tolist())
    for output in graph_model2(batch.to(device)):
        graph_embeddings2.append(output.tolist())
    for output in graph_model3(batch.to(device)):
        graph_embeddings3.append(output.tolist())
    for output in graph_model4(batch.to(device)):
        graph_embeddings4.append(output.tolist())
...
similarity1 =sigmoid_kernel(text_embeddings, graph_embeddings1)
similarity2 =sigmoid_kernel(text_embeddings, graph_embeddings2)
similarity3 =sigmoid_kernel(text_embeddings, graph_embeddings3)
similarity4 =sigmoid_kernel(text_embeddings, graph_embeddings4)
similarity = (similarity1 + similarity2 + similarity3 + similarity4)/4
...
print('================ DONE ================')
similarity1 = cosine_similarity(text_embeddings, graph_embeddings1)
similarity2 = cosine_similarity(text_embeddings, graph_embeddings2)
similarity3 = cosine_similarity(text_embeddings, graph_embeddings3)
similarity4 = cosine_similarity(text_embeddings, graph_embeddings4)
similarity = (similarity1 + similarity2 + similarity3 + similarity4)/4
solution = pd.DataFrame(similarity)
solution['ID'] = solution.index
solution = solution[['ID'] + [col for col in solution.columns if col!='ID']]
